 ...
 * - Description: This class provides access to a list of names stored in a CSV
 *                file. It is designed to be used for generating mock data or
 *                testing scenarios that require realistic person names, genders,
 *                ages, and occupations. The class conforms to the CSVKind
 *                protocol, which defines the structure for accessing CSV-based
 *                data.
 * - Note: This class is only available in DEBUG builds to ensure it's not
 *         included in production code.
 */
class Names: CSVKind {
   /**
    * Load the list of names from the CSV file when the class is first accessed.
    * - Note: The list of names is stored in a CSV file with the following structure:
